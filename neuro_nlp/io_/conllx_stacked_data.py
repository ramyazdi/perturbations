...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
                                  max_size=None, normalize_digits=True, prior_order='deep_first', use_gpu=False, req_grad=True):
    ...
    for bucket_id in range(len(_buckets)):
        ...
        types = torch.LongTensor(tid_inputs)
        masks_e = torch.FloatTensor(masks_e)
        single = torch.LongTensor(single)
        lengths_e = torch.LongTensor(lengths_e)

        stacked_heads = torch.LongTensor(stack_hid_inputs)
        children = torch.LongTensor(chid_inputs)
        siblings = torch.LongTensor(ssid_inputs)
        stacked_types = torch.LongTensor(stack_tid_inputs)
        skip_connect = torch.LongTensor(skip_connect_inputs)
        masks_d = torch.FloatTensor(masks_d)
        lengths_d = torch.LongTensor(lengths_d)

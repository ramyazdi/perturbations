...
class SkipConnectRNNBase(nn.Module):
    ...
    def forward(self, input, skip_connect, mask=None, hx=None):
        ...
        if hx is None:
            ...
            hx = input.data.new(self.num_layers * num_directions, batch_size, self.hidden_size).zero_()
            if self.lstm:
                hx = (hx, hx)
    ...
    def step(self, input, hx=None, hs=None, mask=None):
        ...
        if hx is None:
            hx = input.data.new(self.num_layers, batch_size, self.hidden_size).zero_()
            if self.lstm:
                hx = (hx, hx)
        if hs is None:
            hs = input.data.new(self.num_layers, batch_size, self.hidden_size).zero_()
...
class SkipConnectFastLSTM(SkipConnectRNNBase):
    ...
    .. math::
            ...
            o_t = \mathrm{sigmoid}(W_{io_} x_t + b_{io_} + W_{ho} h_{(t-1)} + b_{ho}) \\
...
class SkipConnectFastLSTMCell(VarRNNCellBase):
    ...
    .. math::
        ...
        o = \mathrm{sigmoid}(W_{io_} x + b_{io_} + W_{ho} h + b_{ho}) \\
        c' = f * c + i * g \\
        h' = o * \tanh(c') \\
        \end{array}
